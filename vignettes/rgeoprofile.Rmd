...
<a><img src='figures/ddf-method.png' align="center" width="675" /></a>
...
```{r, eval=FALSE}
lp <- linear_profile(desalvo$lat, desalvo$lon)
...
```{r, eval=FALSE}
lognorm_profile(desalvo$lat, desalvo$lon)
## Follow the mapping method under the `cgt_profile()` function
```
```{r}
...
```{r, eval=FALSE}
norm_profile(desalvo$lat, desalvo$lon)
## Follow the mapping method under the `cgt_profile()` function
```
```{r}
...
```{r, eval=FALSE}
trun_neg_exp_profile(desalvo$lat, desalvo$lon)
## Follow the mapping method under the `cgt_profile()` function
```
```{r}
...
```{r}
# Note that the mean, geometric mean, and harmonic mean are so close the points appear as overlapping
library(leaflet)
leaflet(width = "100%") %>%
  addProviderTiles('Esri.WorldTopoMap', group = 'Topo') %>%
  addAwesomeMarkers(lng = -71.07357, lat = 42.41322, icon = awesomeIcons(
